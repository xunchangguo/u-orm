...
class BulkAccessorFactory {
	...
	BulkAccessor create0() {
		Class beanClass;
		try {
			ClassFile classfile = make( mgetters, msetters );
	...
	private Object newInstance(Class type) throws Exception {
		...
		instance.getters = mgetters;
		instance.setters = msetters;
		final int len = fields.length;
		instance.fields = new String[len];
//		instance.types = new Class[len];
		for ( int i = 0; i < len; i++ ) {
//			instance.getters[i] = mgetters[i];
//			instance.setters[i] = msetters[i];
