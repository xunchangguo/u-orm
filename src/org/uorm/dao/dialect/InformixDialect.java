...
public class InformixDialect implements Dialect {
	...
	public String getLimitString(String query, int offset, int limit) {
		...
		return new StringBuffer( query.length() + 8 )
				.append( query )
				.insert( query.toLowerCase().indexOf( "select" ) + 6, " first " + limit )
				.toString();
	}
	...
	public String getSequenceNextValString(String tablename) {
		return "select " + getSelectSequenceNextValString( SequenceGenerator.getDefaultSequenceName(tablename) ) + " from informix.systables where tabid=1";
	}
