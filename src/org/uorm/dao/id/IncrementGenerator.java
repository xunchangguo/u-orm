...
public class IncrementGenerator implements IdentifierGenerator {
	...
	public Serializable generate(Dialect dialect, ICommonDao dao, Object pojo,
		...
		try{
			...
			if( Long.class.isAssignableFrom( fieldType ) 
				...
				String sql = "SELECT MAX(" + idFieldInfo.columnName() + ") FROM " + tableName;
				Object idval = dao.querySingleObject(fieldType, sql);
				if(idval == null){
					idval = 0;//fieldType.newInstance();
				}
				if ( Long.class.isAssignableFrom( fieldType ) ) {
					idval = Long.valueOf( ((Number)idval).longValue() + 1 );
				} else if ( Integer.class.isAssignableFrom( fieldType ) ) {
					idval = Integer.valueOf( (int) (((Number)idval).longValue() + 1) );
				}  else if ( Short.class.isAssignableFrom( fieldType ) ) {
					idval = Short.valueOf( (short) (((Number)idval).longValue() + 1) );
				} else if ( BigInteger.class.isAssignableFrom( fieldType ) ) {
					idval = BigInteger.valueOf( ((Number)idval).longValue() + 1 );
				} else if ( BigDecimal.class.isAssignableFrom( fieldType ) ) {
					idval = BigDecimal.valueOf( ((Number)idval).longValue() + 1 );
				}
				if ( writevalue ) {
	...
	public Serializable[] generate(Dialect dialect, ICommonDao dao,
			...
			throws SQLException {
		if(pojos.length <= 1){
			return new Serializable[]{generate(dialect, dao, pojos[0], idFieldInfo, writevalue)};
		}else{
				...
				if( Long.class.isAssignableFrom( fieldType ) 
					...
					if ( Long.class.isAssignableFrom( fieldType ) ) {
						...
						for(int i = 0; i < rtns.length; i ++){
							rtns[i] = lidval + i + 1;
							if ( writevalue ) {
								prop.getWriteMethod().invoke(pojos[i], rtns[i]);
							}
						}
					} else if ( Integer.class.isAssignableFrom( fieldType ) ) {
						...
						short sidval = (short) (((Number)idval).longValue());
						for(int i = 0; i < rtns.length; i ++){
							rtns[i] = sidval + i + 1;
							if ( writevalue ) {
								prop.getWriteMethod().invoke(pojos[i], rtns[i]);
						...
						BigInteger bidval = BigInteger.valueOf( ((Number)idval).longValue() );
						for(int i = 0; i < rtns.length; i ++){
							if(i == 0){
								rtns[i] = bidval.add(BigInteger.ONE);
							}else{
								rtns[i] = ((BigInteger)rtns[i-1]).add(BigInteger.ONE);
							}
							if ( writevalue ) {
								prop.getWriteMethod().invoke(pojos[i], rtns[i]);
							}
						...
						BigDecimal bidval = BigDecimal.valueOf( ((Number)idval).longValue() );
						for(int i = 0; i < rtns.length; i ++){
							if(i == 0){
								rtns[i] = bidval.add(BigDecimal.ONE);
							}else{
								rtns[i] = ((BigDecimal)rtns[i-1]).add(BigDecimal.ONE);
							}
							if ( writevalue ) {
								prop.getWriteMethod().invoke(pojos[i], rtns[i]);
							}
	...
	public Serializable[] generate(Dialect dialect, ICommonDao dao,
			...
			FieldMapping idFieldInfo, boolean writevalue) throws SQLException {
		if(valmaps.length <= 1){
			return new Serializable[]{generate(dialect, dao, cls, valmaps[0], idFieldInfo, writevalue)};
		}else{
